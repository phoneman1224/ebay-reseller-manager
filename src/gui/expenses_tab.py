...
class ExpensesTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.table.setHorizontalHeaderLabels([
            "ID", "Date", "Amount", "Category", "Vendor", 
            "Tax Deductible", "Description", "Actions"
        ])
    ...
    def refresh_data(self):
        ...
        for row, expense in enumerate(expenses):
            ...
            self.table.setItem(row, 4, QTableWidgetItem(expense['vendor'] or ''))
            ...
            self.table.setItem(row, 6, QTableWidgetItem(expense['description'] or ''))
            ...
            """)
            edit_btn.clicked.connect(lambda checked, e=expense: self.edit_expense(e['id']))
    ...
    def view_expense(self, expense):
        ...
        # Get full expense details including inventory items
        full_expense = self.db.get_expense(expense['id'])
        
        # Build inventory items section
        inventory_section = ""
        if full_expense.get('inventory_items'):
            inventory_section = "<h3>Connected Inventory Items:</h3><ul>"
            for item in full_expense['inventory_items']:
                amount_text = f" (Allocated: ${item['allocated_amount']:.2f})" if item.get('allocated_amount') else ""
                inventory_section += f"<li>{item['title']}{amount_text}</li>"
            inventory_section += "</ul>"
        else:
            inventory_section = "<p><i>No inventory items connected</i></p>"
        
        details = f"""
        <h2>Expense Details</h2>
        <p><b>Date:</b> {full_expense['date']}</p>
        <p><b>Amount:</b> ${full_expense['amount']:.2f}</p>
        <p><b>Category:</b> {full_expense['category']}</p>
        <p><b>Vendor:</b> {full_expense['vendor'] or 'N/A'}</p>
        <p><b>Payment Method:</b> {full_expense['payment_method'] or 'N/A'}</p>
        <p><b>Tax Deductible:</b> {'Yes' if full_expense['tax_deductible'] else 'No'}</p>
        <p><b>Description:</b> {full_expense['description'] or 'N/A'}</p>
        <p><b>Notes:</b> {full_expense['notes'] or 'N/A'}</p>
        {inventory_section}
        """
...
class AddEditExpenseDialog(QDialog):
    ...
    def load_expense_data(self):
        ...
        date = QDate.fromString(expense['date'], "yyyy-MM-dd")
        ...
        self.vendor_input.setText(expense['vendor'] or '')
        ...
        self.deductible_check.setChecked(bool(expense['tax_deductible']))
        self.description_input.setPlainText(expense['description'] or '')
        self.notes_input.setPlainText(expense['notes'] or '')
        
        # Load connected inventory items
        if expense.get('inventory_items'):
            self.selected_inventory_items = [dict(item) for item in expense['inventory_items']]
            self.update_inventory_items_display()
    ...
    def save_expense(self):
        ...
        expense_data = {
            ...
            'inventory_items': self.selected_inventory_items
        }
