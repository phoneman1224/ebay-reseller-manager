...
class Database:
    ...
    def get_setting(self, key: str, default: Optional[Any] = None) -> Optional[Any]:
        """Return a persisted setting value.

        Settings are stored as JSON so we can represent complex structures while
        keeping the schema flexible.  Callers that previously expected string
        values will continue to work because plain strings are valid JSON.
        """
    ...
    def get_import_settings(self) -> Dict[str, Any]:
        """Return the user's import/settings configuration."""

        value = self.get_setting("import_settings", default={})
        if isinstance(value, dict):
            return value
        return {}

    def update_import_settings(self, settings: Dict[str, Any]) -> None:
        """Persist the combined import/settings configuration."""

        if settings is None:
            settings = {}
        self.set_setting("import_settings", settings)
